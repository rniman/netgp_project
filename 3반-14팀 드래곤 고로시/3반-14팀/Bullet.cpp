...
void CheckStage2(BLinkedList* bullet, MonsterStage2 Mob[])
{
	...
	while (cus != NULL) {
		...
		for (int i = 0; i < 4; ++i) {
			...
			if (IntersectRect(&temp, &cus->data.hitBox, &Mob[i].rect)) {
				...
				if (Mob[i].Hp <= 0){
					Mob[i].ALIVE = FALSE; Mob[i].Explode = TRUE; Mob[i].Idlecount = 0;
				}
				cus->data.exist = FALSE;
				break;
			}
...
void CheckStage4(BLinkedList* bullet, STAGE4SPHERE* MeteorHead, STAGE4EVENT* Stage4Event, BOOL* potalOn)
{
	...
	while (cus != NULL) {
		...
		for (STAGE4SPHERE* i = MeteorHead; i != NULL; i = i->next) {
			...
			if (IntersectRect(&temp, &cus->data.hitBox, &i->rect) && i->dis == 0) {
				i->extinction = TRUE;
		...
		if (Stage4Event->STAGE4CLEAR) {
			if (IntersectRect(&temp, &cus->data.hitBox, &Stage4Event->rect)) {
				Stage4Event->HIT = TRUE;
				cus->data.exist = FALSE;
				*potalOn = TRUE;
				break;
			}
